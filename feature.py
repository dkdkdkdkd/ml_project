...
def display(img_dir, dst, new_root):
	...
	new_pure_img_dir = os.path.join(new_root, 'pure', img_name)
	...
	for i in range(len(lines)):
		...
		cv2.circle(a, (x, y), radius, (0, 0, 0), -1)
	# cv2.imshow('img', a)
	# cv2.waitKey(0)
	cv2.imwrite(new_img_dir, img)
	cv2.imwrite(new_pure_img_dir, a)

	return 1

	
def main():
	src = './SCUT-FBP5500_v2/facial landmark'
	dst = './SCUT-FBP5500_v2/landmark_txt'
	...
	jpg_root = './SCUT-FBP5500_v2/Images'
	new_root = './SCUT-FBP5500_v2/Save'
	os.mkdir(new_root) if not os.path.exists(new_root) else None
	os.mkdir(new_root + "/pure") if not os.path.exists(new_root + "/pure") else None
